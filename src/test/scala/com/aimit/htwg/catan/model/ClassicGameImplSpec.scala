...
import com.aimit.htwg.catan.model.state.{ ActionState, InitState }
...
class ClassicGameImplSpec extends AnyWordSpec with Matchers {
  ...
  "ClassicGameImpl" when {
    ...
    val randomGame:ClassicGameImpl = new ClassicGameImpl( ClassicGameFieldImpl( seed ), ClassicTurnImpl(), seed, ( pID:PlayerID, color:PlayerColor, name:String ) => ClassicPlayerImpl( pID, color, name ), "ClassicPlayerImpl" )
    "random new" should {
      "have state" in {
