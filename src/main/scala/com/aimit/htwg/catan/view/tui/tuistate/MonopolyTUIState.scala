...
case class MonopolyTUIState( controller:Controller ) extends TUIState {
  ...
  override def getActionInfo:String = "Type [<" + Resource.impls.map( _.name ).mkString( "|" ) + ">] to specify a resource"

  override def inputPattern:Option[String] =
    Some( "(" + Resource.impls.map( r => TUI.regexIgnoreCase( r.name ) ).mkString( "|" ) + ")" )
