...
object Card extends ObjectComponent[Card]:
  ...
  given resourceCardsReads:Reads[ResourceCards] = ( json:JsValue ) => JsSuccess( json.asMap[Resource, Int] )
  ...
  implicit class RichResourceCards( resources:ResourceCards ):
    ...
    def has( requiredResources:ResourceCards ):Boolean =
      requiredResources.foreach( data => if !resources.contains( data._1 ) || resources( data._1 ) < data._2 then return false )
      true
...
object DevelopmentCard extends ObjectComponent[DevelopmentCard]:
  ...
  given devCardWrites:Writes[DevelopmentCard] = ( developmentCard:DevelopmentCard ) => Json.toJson( developmentCard.title )
  given devCardsReads:Reads[DevelopmentCard] = ( json:JsValue ) => JsSuccess( of( json.as[String] ).get )
...
object BonusCard extends ObjectComponent[BonusCard]:
  given bonusCardWrites:Writes[BonusCard] = ( bonusCard:BonusCard ) => Json.toJson( bonusCard.title )
  given bonusCardReads:Reads[BonusCard] = ( json:JsValue ) => JsSuccess( BonusCard.of( json.as[String] ).get )
