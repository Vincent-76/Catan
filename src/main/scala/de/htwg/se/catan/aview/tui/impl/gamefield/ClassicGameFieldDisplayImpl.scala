...
case class ClassicGameFieldDisplayImpl(
  ...
  def buildGameField:String =
    ...
    field.view.zipWithIndex.map( d => {
      TUI.reset + d._1.mkString( TUI.reset ) + (if d._2 - 2 >= 0 && d._2 - 2 < legend.size then
  ...
  private def buildHex( f:Field, h:Hex, i:Int, j:Int ):Field =
    val base = (2 to 4).red( f, ( field:Field, oi:Int ) => (1 to 7).red( field, ( fi:Field, oj:Int ) => {
      ...
      .update( i + 2, j + 4, ClassicGameFieldDisplayImpl.colorOf( h.area.f ) + (if h == game.gameField.robberHex then
        (h.area.f match
    ...
    val f1 = h.area match
      ...
      case a:WaterArea =>
        if a.port.isDefined then
          if a.port.get.specific.isDefined then
      ...
      case a:ResourceArea =>
        (if buildableIDs.isDefined && buildableIDs.get.contains( h ) then
  ...
  private implicit class RichField( val f:Field ):
    ...
    def addEdge( h1:Hex, hex2:Option[Hex], i:Int, j:Int, dir:EdgeDir ):Field =
      if hex2.isEmpty then
        return f.update( i, j, dir.symbol )
      val edge = gameField.findEdge( h1, hex2.get )
      if edge.isEmpty then
        return f.update( i, j, dir.symbol )
      if h1.isWater && hex2.get.isWater then
        return f.update( i, j, colorOf( Water ) + " " )
      val f1 = if edge.get.road.isEmpty then
        if isShowingID( buildableIDs, edge.get ) then
      ...
      if edge.get.port.isEmpty then
        return f1
      val c = if hex2.get.area.isInstanceOf[WaterArea] then 1 else -1
      val (o1, o2) = if buildableIDs.isEmpty then
        dir match
    ...
    def addVertex( h1:Hex, hex2:Option[Hex], hex3:Option[Hex], i:Int, j:Int ):Field =
      if hex2.isEmpty || hex3.isEmpty then
        if i == 0 || i == f.length - 1 then
          return f.update( i, j, " " )
        else
          return f.update( i, j, "|" )
      ...
      if vertex.isEmpty then
        return f.update( i, j, " " )
      if vertex.get.building.isEmpty then
        if isShowingID( buildableIDs, vertex.get ) then
  ...
  private implicit class RichPortOffsets( val o:PortOffsets ):
    def checkIDs( ids:Boolean ):PortOffsets =
      if !ids then return o
      ((c( o._1._1 ), c( o._1._2 )), (c( o._2._1 ), c( o._2._2 )))

    private def c( i:Int ):Int = if i.abs == 2 then i / 2 else i
