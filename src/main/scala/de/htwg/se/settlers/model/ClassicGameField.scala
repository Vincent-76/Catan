...
import de.htwg.se.settlers.model.ClassicGameField.{Edges, Hexagons, Vertices, adjacentOffset, nextAdjacentOffset}
...
case class ClassicGameField(
  ...
  /*override*/ def findHex( hID:Int ):Option[Hex] = {
    hexagons.foreach( _.foreach( h => if ( h.isDefined && h.get.id == hID ) return h ) )
    Option.empty
  }

  /*override*/ def findHex( r:Int, c:Int ):Option[Hex] = ClassicGameField.findHex( r, c, hexagons )

  /*override*/ def adjacentHexes( h:Hex ):List[Hex] = ClassicGameField.adjacentHexes( h, hexagons )

  /*override*/ def adjacentHex( h:Hex, ai:Int ):Option[Hex] = {
  ...
  /*override*/ def findEdge( eID:Int ):Option[Edge] = edges.values.find( _.id == eID )

  /*override*/ def findEdge( h:(Hex, Hex) ):Option[Edge] = ClassicGameField.findEdge( h._1, h._2, edges )

  /*override*/ def adjacentHexes( e:Edge ):List[Hex] = List( e.h1, e.h2 )

  private def enclosingHexes( e:Edge ):List[Hex] = adjacentHexes( e.h1 ).intersect( adjacentHexes( e.h2 ) )

  /*override*/ def adjacentEdges( e:Edge ):List[Edge] = {
  ...
  /*override*/ def adjacentVertices( e:Edge ):List[Vertex] = {
    val enclosedHexes = enclosingHexes( e )
    /*if ( enclosedHexes.isEmpty )
      return List.empty*/
  ...
  /*override*/ def update( e:Edge ):ClassicGameField = copy( edges = edges.updated( (e.h1, e.h2 ), e ) )


  /*override*/ def findVertex( vID:Int ):Option[Vertex] = vertices.values.find( _.id == vID )
  ...
  /*override*/ def findVertex( h:(Hex, Hex, Hex) ):Option[Vertex] = vertices.get( sortVertexKey( h._1, h._2, h._3 ) )

  /*override*/ def adjacentEdges( v:Vertex ):List[Edge] = findEdge( v.h1, v.h2 ).toList ++ findEdge( v.h1, v.h3 ).toList ++ findEdge( v.h2, v.h3 )

  /*override*/ def update( v:Vertex ):ClassicGameField = copy( vertices = vertices.updated( (v.h1, v.h2, v.h3), v ) )
}

object ClassicGameField {

  def apply( random:Random = Random ):ClassicGameField = {
    ...
    ClassicGameField( hexagons, edges, createVertices( hexagons, edges ), robber )
  }
