...
import de.htwg.se.settlers.ui.gui.{ GUI, GUICommand, GUIState, GUIApp }
  ...
  override def getActions:List[GUICommand] = List( new GUICommand {
    override def getNode( gui:GUI ):Node = new BorderPane {
      ...
      center = new VBox {
        ...
        children = state.decisions.filter( _._2 ).map( d => new VBox {
          ...
          children = List(
            ...
            new Button( "Trade" ) {
              onAction = _ => controller.game.state.playerTrade( d._1 )
            }
          )
        } )
      }
      bottom = new Button( "Abort" ) {
        alignmentInParent = Pos.Center
        onAction = _ => controller.game.state.abortPlayerTrade()
      }
    }
  } )
