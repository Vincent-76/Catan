package de.htwg.se.settlers.aview.tui.impl.gamefield
...
object ClassicGameDisplayImpl {
  ...
  def apply( controller:Controller, buildableIDs:List[PlacementPoint] ):ClassicGameDisplayImpl = {
    new ClassicGameDisplayImpl( controller.game.asInstanceOf[ClassicGameImpl], controller.game.gameField.asInstanceOf[ClassicGameFieldImpl], Some( buildableIDs ) )
  }

  def apply( controller:Controller ) = new ClassicGameDisplayImpl( controller.game.asInstanceOf[ClassicGameImpl], controller.game.gameField.asInstanceOf[ClassicGameFieldImpl] )
}

class ClassicGameDisplayImpl( game:ClassicGameImpl, gameField:ClassicGameFieldImpl, placement:Option[List[PlacementPoint]] = None ) extends GameDisplay[ClassicGameFieldImpl] {
  ...
  implicit class RichField( val f:Field ) {
    ...
    def fillEmptyArea( h:Hex, i:Int, j:Int ):Field = f.update( i, j, ClassicGameDisplayImpl.colorOf( h.area.f ) + " " )
    ...
    def addEdge( h1:Hex, hex2:Option[Hex], i:Int, j:Int, dir:EdgeDir ):Field = {
      ...
      if ( h1.isWater && hex2.get.isWater )
        return f.update( i, j, ClassicGameDisplayImpl.colorOf( Water ) + " " )
      ...
      f1.update( i + o1._1 * c, j + o1._2 * c, ClassicGameDisplayImpl.colorOf( Water ) + TUI.textOnColor + ClassicGameDisplayImpl.port )
        .update( i + o2._1 * c, j + o2._2 * c, ClassicGameDisplayImpl.colorOf( Water ) + TUI.textOnColor + ClassicGameDisplayImpl.port )
    }
    ...
    def addVertex( h1:Hex, hex2:Option[Hex], hex3:Option[Hex], i:Int, j:Int ):Field = {
      ...
      if ( vertex.get.building.isEmpty )
        if ( isShowingID( vertex.get ) )
          ...
          return f.update( i, j, ClassicGameDisplayImpl.emptyVertex )
      ...
      vertex.get.building.get match {
        case _:Settlement => f.update( i, j, TUI.colorOf( player.color ) + ClassicGameDisplayImpl.settlement )
        case _:City => f.update( i, j, TUI.colorOf( player.color ) + ClassicGameDisplayImpl.city )
  ...
  private def getLegend:Vector[(String, String)] = {
    val legend = game.resourceStack.red( ClassicGameDisplayImpl.legend :+ ("", ""), ( l:Vector[(String, String)], r:Resource, amount:Int ) => {
  ...
  private def buildHex( f:Field, h:Hex, i:Int, j:Int ):Field = {
    val base = ( 2 to 4 ).red( f, ( field:Field, oi:Int ) => ( 1 to 7 ).red( field, ( fi:Field, oj:Int ) => {
      ...
      .update( i + 2, j + 4, ClassicGameDisplayImpl.colorOf( h.area.f ) + ( if ( h == game.gameField.robberHex )
        ( h.area.f match {
          case Desert => TUI.text
          case _ => TUI.textOnColor
        } ) + ClassicGameDisplayImpl.robber
      else " " ) )
    val f1 = h.area match {
      ...
      case a:WaterArea => if ( a.port.isDefined )
        if ( a.port.get.specific.isDefined )
          base.showOnFields( a.port.get.specific.get.title, i + 3, j + 4, TUI.textOnColor + ClassicGameDisplayImpl.colorOf( a.port.get.specific.get ) )
        else
      ...
      case a:ResourceArea =>
        ...
        else base ).showOnFields( a.number.value.toString, i + 3, j + 4, ClassicGameDisplayImpl.colorOf( h.area.f ) + TUI.textOnColor )
          .showOnFields( h.area.f.title, i + 4, j + 4, ClassicGameDisplayImpl.colorOf( h.area.f ) + TUI.textOnColor )
    }
  ...
  def buildPlayerDisplay( turnPlayer:Option[PlayerID] = None ):String = {
    ...
    val s = otherPlayers.map( p => {
      TUI.displayName( p, nameLength ) +
        ...
        " UsedDevCards[" + Cards.devCards.flatMap( c => ( 0 until p.usedDevCards( c ) ).map( _ => c ) ).map( _.title ).mkString( "|" ) + "] " +
